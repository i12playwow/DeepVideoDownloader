...
- **Built-in browser (added 8/2026)**: the app can open a browser window that runs the Deep Grab extension in its own Electron session (`persist:deepgrab-browser`), so videos are captured/downloaded without a separate Chrome. `main.js`: `loadBrowserExtension()` (uses `config.extensionPath || __dirname/extension`), `createBrowserWindow()`, IPC `browser-open/nav/back/forward/reload`; UI: `browser-start.html` + a URL bar in the renderer. **The extension must use `chrome.storage.local`, not `sync`** — Electron has no `storage.sync`, and content.js errors at init (no toolbar) if it references it (verified 8/2026). Only one extension per partition; reload the app after editing the extension.
