...
- No lint/test/typecheck. Syntax check all JS: `npm.cmd run check` (runs `node --check` on root + `lib/` + extension + test files). Individual file: `node -c <file>.js`.
...
- `downloader.js` — `DownloadManager` engine (segmented, queue, pause/resume/cancel, retry, **global** speed limit, scheduling, history, thumbnails, bandwidth stats). Helpers split into `lib/`: `lib/resolvers.js` (site regexes + `resolve*()` + `resolveUrl` dispatcher), `lib/http.js` (`fetchHtml`, `requestWithRedirects`, `contentRange*`), `lib/hls.js` (playlist parse, variant pick, PNG strip), `lib/errors.js` (failure classification), `lib/names.js` (`sanitizeName`, `titleFromReferer`). `downloader.js` re-exports the full public API — `require("./downloader")` callers (main.js, tests) are unchanged. History → `<downloadDir>\history.json` (moves with the dir). For a normal single download `done` items stay in the active map until dismissed; bulk `addPending` auto-moves completed to history. Persistence debounced ~500ms via `_persistSoon()`; `flush()` writes synchronously on quit. **Bulk-scale**: `downloads-add-many` → `addPending()` keeps bounded live window (`config.liveWindow || concurrency*4`); completed items auto-move to history (`_toHistory`, cap `config.maxHistory` default 2000). `downloaded.json` set in download dir records done URLs; `skipDuplicates` creates `status:"duplicate"` rows (re-download via `forceDownload(id)`). Cloudflare/anti-bot: 429/`cf-chl`/`turnstile` → rate-limited/blocked → proxy rotation + backoff; expired errors (403/404/410, `expired|token|forbidden`) also rotate proxy (`markBad` + clear `item._proxy`) before refresh re-resolve.
...
- `missav.(ws|ai|com|live|xyz)` (incl. numbered subdomains like `missav123.com`) → `resolveMissav` (`lib/resolvers.js`) fails fast with a `requires-browser` error instead of fetching — MissAV is anti-bot/Cloudflare and the Node downloader has no cookie support (see HLS downloads). The built-in Browser + extension must capture the m3u8.
...
- **Cloudflare cookie bridge**: for downloads enqueued from the built-in browser (WS `download` from the extension), `main.js` injects a `cookieProvider` (`cookieHeaderFor`) into `DownloadManager` that queries the `persist:deepgrab-browser` session **per request URL**, so every playlist/segment fetch carries the `cf_clearance`/session `Cookie` for *that* host — including a separate Cloudflare CDN serving segments. A static `item.cookieHeader` captured at enqueue (`gatherCookieHeader`) is the fallback when the provider returns nothing. All request paths use `downloader.js` `_reqHeaders(item, baseHeaders, url, extra)` so the cookie is resolved per URL. Caveats: cookies still come only from hosts the browser actually visited (a host the browser never loaded still 403s) and aren't refreshed mid-download; direct (non-browser) URLs get no cookies. Cookies are never persisted (`public()`/`_toHistory` exclude them).
- **Browser user-agent**: `main.js` `browserSession()` sets a real Chrome UA (`BROWSER_UA`) on the `persist:deepgrab-browser` partition. Anti-bot/Cloudflare sites (e.g. missav*) serve a stripped "verify you are human" **partial page** to the default `Electron/<ver>` UA, so without this the built-in browser renders incompletely and the m3u8 never loads. The UA only affects webview/extension requests — the Node downloader's own `lib/http.js` requests use their own UA and are untouched.
- **Auto-click Cloudflare checkbox**: `extension/content.js` (runs in every frame via `all_frames:true`, so it reaches the cross-origin challenge iframe) polls for a Cloudflare "Verify you are human" challenge and clicks the checkbox/button automatically (stops after ~45s or once the challenge clears). Best-effort — some managed Turnstile challenges still need a real user gesture, in which case the user clicks manually.
...
- Extension `bestOnly` defaults ON and `tryCapture` drops HLS from DOM scanning (`extension/content.js`), but a `.m3u8` captured via the background `webRequest` (`NET_VIDEO_RE`) IS surfaced in the overlay even in best-only mode (so Cloudflare/HLS-only sites like missav* are addable without toggling Best only off). `minSizeMB` filtering still compares against the playlist file size (a few KB), never the assembled video — filtering for "100MB+" hides every HLS entry, since HLS total size is unknown before segments download.
