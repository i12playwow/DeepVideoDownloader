...
## HLS downloads
`.m3u8` → `runHls()`: fetch playlist → best variant (`pickHlsVariant`) → sequential segment download → `remuxToMp4` via ffmpeg (`-c copy -bsf:a aac_adtstoasc -movflags +faststart`). Anti-bot decoy: tiktokcdn `.image` URLs prepend fake 1×1 PNG to each TS segment → `stripPngPrefix` trims. ffmpeg output is `<final>.part` — pass `-f mp4` immediately before output arg.
