...
## Gotchas
- **Two repo copies exist**: `C:\Users\SOKCHHORN PC\Desktop\...` (canonical, full — has `extension/`, `extension-firefox/`, `userscripts/`, `run.bat`, all `test-*.js`/`.user.js`, `build/`, `dist/`) vs `...\OneDrive\Desktop\...` (the opencode working copy — a **subset**: core JS only, no extension/tests/userscripts/`run.bat`/`build`/`dist`). Sections below (Verification, extension, Userscripts) describe files present only in the canonical copy; in the OneDrive mirror those files are absent by design. Verify which copy you're editing and stage edits in the canonical one.
