...
async function refreshActiveDir() {
  try {
    const r = await window.api.getActiveDir();
    if (r && r.dir) {
      let extra = "";
      if (typeof r.freeMB === "number") {
        extra = r.freeMB >= 1024 ? " (" + (r.freeMB / 1024).toFixed(1) + " GB free)" : " (" + Math.round(r.freeMB) + " MB free)";
      }
      $("activeDir").textContent = "Saving to: " + r.dir + extra;
    }
  } catch (e) { /* ignore */ }
}
refreshActiveDir();
function updateWsBadge(d) {
  const el = $("wsBadge");
  if (!el) return;
  const n = d && d.count ? d.count : 0;
  el.textContent = n ? (n === 1 ? "● extension connected" : "● " + n + " clients connected") : "no client";
  el.className = "ws-badge " + (n ? "on" : "off");
}
window.api.wsClients().then(updateWsBadge);
window.api.onWsClients(updateWsBadge);
const parseUrls = () => {
