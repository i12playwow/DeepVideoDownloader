...
(() => {
  ...
  async function extractCnPorn() {
    ...
    for (const u of embeds) {
      ...
      try {
        ...
        // route through tryCapture so best-only mode (skip HLS, rank, replace
        // the current best, auto-download) applies to cnporn embeds too
        tryCapture(url, title);
    ...
    chrome.runtime.sendMessage({ type: "add-to-list", url, title: v.title, pageUrl: location.href })
      .then((resp) => {
        if (resp && resp.ok) {
          ...
          // desktop app offline / send failed — stash and flush on the next online poll
          autoPending.add(url);
          v.added = false;
        }
  ...
  function loadConfig() {
    chrome.storage.local.get({ dv: {} }, (data) => {
      config = { ...DEFAULT_CONFIG, ...((data && data.dv) || {}) };
