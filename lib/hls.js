...
const HLS_IFRAMES_ONLY_RE = /#EXT-X-I-FRAMES-ONLY/i;
const QUALITY_DIR_RE = /(?:\d{3,4}p|4k|8k|qhd|wqhd|hls|sd|hd|uhd|fhd|v\d+|default|low|high|medium|index)/i;
...
// Variants from a master playlist, ranked best-first (same scoring the old
// single pick used: RESOLUTION height dominates, BANDWIDTH breaks ties, and
// the first block wins on exact ties — the stable sort keeps that order).
function pickHlsVariants(text, baseUrl) {
  const out = [];
  ...
  for (const block of blocks) {
    ...
    // The variant URI is the first non-comment line after the tag. Walking
    // lines (instead of taking the next line verbatim) keeps blank lines and
    // stray tags between the tag and its URI from either misreading a tag as
    // the URI or dropping a recoverable block.
    const rest = tagEnd === -1 ? "" : block.slice(tagEnd + 1);
    let uriLine = "";
    ...
    out.push({ url: new URL(uriLine, baseUrl).href, score });
  }
  out.sort((a, b) => b.score - a.score);
  return out.map((v) => v.url);
}

function pickHlsVariant(text, baseUrl) {
  const variants = pickHlsVariants(text, baseUrl);
  return variants.length ? variants[0] : null;
}

function isIFrameOnlyPlaylist(text) {
  return HLS_IFRAMES_ONLY_RE.test(String(text || ""));
}
...
function matchHlsMaster(url) {
  try {
    ...
    const dirIsQuality = QUALITY_DIR_RE.test(dir);
    ...
    const stemMaster = /^(.+?)[_-](?:\d{3,4}p|\d{3,4}x\d{3,4}|hevc|avc|hls|sd|hd|uhd|fhd|4k|8k|qhd|wqhd|low|high|medium|default)$/i.exec(name);
...
module.exports = { HLS_RE, HLS_MASTER_RE, HLS_AES_RE, QUALITY_DIR_RE, isHlsUrl, stripPngPrefix, parseHlsPlaylist, pickHlsVariant, pickHlsVariants, isIFrameOnlyPlaylist, matchHlsMaster, isAdSegmentUrl, classifySegments };