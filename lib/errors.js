...
// True when a fetched page is a Cloudflare (or CF-style) challenge interstitial
// rather than real site content. Detection via title/container markers and the
// challenge bootstrap (`_cf_chl_opt`) — modern templates say "Checking your
// browser..." instead of "Just a moment", and classic checks miss them.
...
function isCfChallengeHtml(html) {
  return !!(html && CF_PAGE_RE.test(String(html)));
}
...
// True when a response Content-Type says "this is a web page, not a video
// stream" — used by probe() to fail fast so HTML never gets saved as .mp4 junk
// and marked downloaded. Real video containers are audio/*, video/*,
// application/octet-stream, or missing; text/html never is one.
function isHtmlContentType(ctype) {
  ctype = String(ctype || "").split(";")[0].trim().toLowerCase();
  return ctype === "text/html" || ctype === "application/xhtml+xml";
}

// True when leading stream bytes look like an HTML page (defense-in-depth for
// servers whose HEAD hides the real body as octet-stream or no Content-Type).
const HTML_HEAD_RE = /^(?:[\uFEFF\u00EF\u00BB\u00BF])*(?:<!doctype|<html|<head|<body|<title|<\?xml)/i;

function looksLikeHtmlHead(buf) {
  if (!buf || !buf.length) return false;
  return HTML_HEAD_RE.test(buf.subarray(0, Math.min(buf.length, 32)).toString("latin1"));
}

function notVideoError(detail) {
  const err = new Error("Not a video: " + detail);
  err.category = "not-video";
  return err;
}

module.exports = { isExpiredError, isProxyFailure, isRateLimited, isCloudflareBlocked, isCfChallengeHtml, categorizeError, isHtmlContentType, looksLikeHtmlHead, notVideoError };