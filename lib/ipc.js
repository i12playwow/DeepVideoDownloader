...
// main.js owns: { ipcMain, shell, dialog, dm, getConfig, setConfig,
...
// activeBrowserId, ...) from main.js scope; setConfig validates + persists
// the merged config and rebuilds proxyManager/dm state there too.
"use strict";
const fs = require("fs");
...
function registerIpc(ctx) {
  const { ipcMain, shell, dialog, dm, getConfig, setConfig, getProxyManager, browser, bv } = ctx;
...
ipcMain.handle("settings-save", (e, next) => {
  ...
  setConfig({ ...getConfig(), ...clean }); // validate + persist + rebuild pm/dm
  return { ok: true };
});
