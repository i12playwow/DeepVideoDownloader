...
// supjav.com (NOT supremejav.com — different site, unverified) serves its movie
// pages (https://supjav.com/452555.html) behind a Cloudflare *managed* Turnstile
// which neither headless Node nor the Electron built-in browser can clear: the
// probe returns 403 "Just a moment..." with turnstile/managed flags every time.
// Every such enqueue therefore falls through to a stuck built-in tab and a
// requires-browser error. Keep the direct page out of the queue entirely — the
// ONLY working route is the extension relaying the supjav.php?l= player URL out
// of a CF-cleared real-Chrome tab (that URL does NOT match this predicate).
function isCfwalledSupjavMovie(u) {
  try {
    const p = new URL(u);
    const host = p.hostname.toLowerCase().replace(/^www\.|^m\./, "");
    if (host !== "supjav.com") return false;
    return /\/\d{3,}\.html?$/i.test(p.pathname);
  } catch (e) {
    return false;
  }
}
...
async function resolveJavAggregator(videoPageUrl, { proxyManager, config, paceHost, fetchHtml: injectedFetch }, baseHeaders = {}) {
  ...
  //    The >=3 fallback exists for list pages the prefix rule misses (custom
  //    slugs), but a MOVIE page also carries >=3 related-thumbnail links — so it
  //    would be misread as a list, self-mark "done" with no bytes, and spawn a
  //    wave of child enqueues. Only expand when the URL itself does not look
  //    like a single movie page.
  ...
  const isMovieLike = isSextb ? isSextbMovieUrl(videoPageUrl) : isSupjavMovieUrl(videoPageUrl);
  if (page1Links.length && (listUrlCheck || (!isMovieLike && page1Links.length >= 3))) {
    const list = isSextb
  ...
  if (isMovieLike) {
    // A fetched MOVIE page that yields no source is almost always a JS player or
    // an invisible anti-bot gate — the built-in Browser + Deep Grab extension can
    // capture it, so route there instead of failing silently. Message keeps "cf-"
    // out so downloader.js' Cloudflare retry loop is not triggered.
    const err = new Error("JAV site: no supjav iframe or direct m3u8/mp4 source found — open it in the built-in Browser to capture the stream.");
    err.category = "requires-browser";
    throw err;
  }
