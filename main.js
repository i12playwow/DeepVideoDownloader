...
function broadcastWsClients() {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  const count = wss ? wss.clients.size : 0;
  mainWindow.webContents.send("ws-clients", { count });
}
...
function startWsServer() {
  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("error", () => {});
    ws.on("close", () => broadcastWsClients());
  });
...
ipcMain.handle("get-active-dir", () => {
  const dir = dm.dir;
  let freeMB = null;
  try { const s = fs.statfsSync(dir); freeMB = (s.bavail * s.bsize) / (1024 * 1024); } catch (e) {}
  return { dir, freeMB };
});

ipcMain.handle("ws-state", () => ({ count: wss ? wss.clients.size : 0 }));
